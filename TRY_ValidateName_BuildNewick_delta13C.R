...
TRYdata1 <- rtry_import(paste(inpath,"TRY_RawData/32330.txt",sep=""))
...
Trait = Trait[which(Trait$TraitID == 89),]
datatypes = unique(Trait$DataName)
keeps = c("Leaf delta 13C (Leaf carbon isotope ratio)")
...
#4. Write Newick Tree
write.tree(phylotree$scenario.3,paste(inpath,"TRY_Values_NewickTrees/TRY_delta13C_Newick.txt",sep=""))
...
write.table(TraitExport,paste(inpath,"TRY_Values_NewickTrees/TRY_delta13C_Values.csv",sep=""),row.names = FALSE,sep=",",quote = FALSE)
