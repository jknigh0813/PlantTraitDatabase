...
Trait$XFTdata.P88..MPa. = as.numeric(Trait$XFTdata.P88..MPa.)
...
Trait = Trait[which(!is.nan(Trait$XFTdata.P88..MPa.)),]
Trait = Trait[which(!is.nan(Trait$XFTdata.Cleaned.binomial)),]
Trait = Trait[which(Trait$XFTdata.P88..MPa. <= -0.75),]
Out = data.frame(Trait$XFTdata.rowID,Trait$XFTdata.Cleaned.binomial,Trait$XFTdata.P88..MPa.,Trait$XFTdata.rowID,Trait$XFTdata.rowID)
...
#4. Write Newick Tree
write.tree(phylotree$scenario.3,paste(inpath,"TRY_Values_NewickTrees/XFT_P88_Newick.txt",sep=""))
...
write.table(TraitExport,paste(inpath,"TRY_Values_NewickTrees/XFT_P88_Values.csv",sep=""),row.names = FALSE,sep=",",quote = FALSE)
